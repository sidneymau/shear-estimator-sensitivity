...
# def moffat_psf_combinations(pixel_scale):
#     """
# 
#     Generates combinations with:
# 
...
# 
#                 combinations.append((original_gal, 0.0, 0.0, true_psf, deconv_psf, reconv_psf, reconv_psf, delta_g, delta_g, 'REGAUSS', pixel_scale))
# 
#     return combinations


def response_accuracy_test_gaussian(pixel_scale):
    """
...
def response_accuracy_test_moffat(pixel_scale):
    """
    ...
    gal_psf_ratios = np.arange(0.5, 2.1, 0.1)
    true_psf_fwhm = np.arange(0.5, 1.3, 0.1)  # 0.5 - 1.3 arcseconds fwhm converted to sigma
    

    combinations = []

	# excluding values of zero because they create problems
    oshear_dgs = [i for i in np.arange(-0.05, 0.06, 0.01) if not abs(i) < 0.001]
        ...
        for i in range(len(true_psf_fwhm)):
            for cshear_delta_g in cshear_dg:
                for oshear_dg in oshear_dgs: 
                    ...
                    true_psf = galsim.Moffat(flux=gal_flux, beta=psf_beta, fwhm=true_psf_fwhm[i])
                    deconv_psf = true_psf
                    reconv_psf = galsim.Moffat(flux=gal_flux, beta=psf_beta, fwhm=dilation_factor * true_psf_fwhm[i])
            ...
            for cshear_delta_g in cshear_dg:
                for oshear_dg in oshear_dgs:
                    for pixel_scale in [0.2, 0.02]:
                        ...
                        true_psf = galsim.Moffat(flux=gal_flux, beta=psf_beta, fwhm=true_psf_fwhms[i])
                        deconv_psf = true_psf
                        reconv_psf = galsim.Moffat(flux=gal_flux, beta=psf_beta, fwhm=dilation_factor*true_psf_fwhms[i])
...
def response_accuracy_test_better_moffat_REGAUSS_LINEAR_BJ():
    """
    ...
    - variable dilation factor (from Huff & Mandelbaum 2017)
    - variable pixel_scale (0.2 and 0.02))
    - REGAUSS, LINEAR, and BJ shape measurement algorithms.
    ...
    shape_mes_algs = ['REGAUSS', 'LINEAR', 'BJ']    
    
    for shape_mes_alg in shape_mes_algs:
        for ratio in gal_psf_ratios:        # gal_fwhm / psf_fwhm should be equal to ratio
            for i in range(len(true_psf_fwhms)):
                for cshear_delta_g in cshear_dg:
                    for oshear_dg in oshear_dgs:
...
def response_accuracy_test_better_gaussian_REGAUSS_LINEAR_BJ():
    """
    ...
    - variable dilation factor (from Huff & Mandelbaum 2017)
    - variable pixel_scale (0.2 and 0.02))
    - REGAUSS, LINEAR, and BJ shape measurement algorithm.
